##
## "THE SCRIPT" is the product of many months of work and includes (if you select them):
##
...
##
## Regards - Peter Scargill  https://tech.scargill.net  - See also: BigTimer and ESP-GO
##
## IMPORTANT:-
...
## 3. This script could take 3+ hours on a slow Pi Zero Wifi.... steer away from midnight to avoid any updates
##    such as dietpi upgrades etc.
...
##
## We cannot answer questions on board/operating system comninations we have not tested. See notes below for tested combos.
## 
## 20/08/2018 Fixed Java and settings.js generation, added menu option (default: UNCHECKED) to enable/disable HW support (I2C, GPIO, Serial, etc)
## 01/05/2018 Redone InfluxDB/Grafana/Chronograf install, now works on every platform tested, and succesfully tested on Rock64+eMMc
## 05/10/2017 Added glances - use directly - glances - or as webserver glances -w
## 08/09/2017 Added name change for Node-Red flows files - and tested Roseapple Pi with DietPi successfully
## 31/08/2017 Scrapped JED as MC is much better - added rpi-clone which works a treat with NANOPI
...
## 04/01/2017 Tested DietPi on Orange Pi Zero - installed perfectly - 1 hour. WIFI still iffy
...
if [[ $MYMENU == *"prereq"* ]]; then
    ...
    sudo apt install $AQUIET -y sysfsutils
...
if [[ $MYMENU == *"mosquitto"* ]]; then
    ...
    if [[ $OPSYS == *"UBUNTU"* ]]; then
		...
		if [[ $OPSYS == *"BIAN"* ]] && [[ $DISTRO == *"9."* ]]; then
			printl "${IRed}!!!! Installing stock Mosquitto from Debian 9! ${IWhite}\r\n"
		else
...
if [[ $MYMENU == *"wiringpi"* ]]; then
    ...
    cd
    git clone git://git.drogon.net/wiringPi 2>&1 | tee -a $LOGFILE
    # if git fails or folder does not exist, abort+report
    if [ $? -eq 0 ] && [ -d wiringPi ]; then
        cd ~/wiringPi
...
if [[ $MYMENU == *"apache"* ]]; then
    ...
    if [ $apache -eq 1 ] && [ -d /var/www/html ]; then
        ...
        if [ $? -eq 0 ]; then
            sudo unzip phpLiteAdmin_v1-9-7-1.zip 2>&1 | tee -a $LOGFILE
            sudo mv phpliteadmin.php index.php
            sudo mv phpliteadmin.config.sample.php phpliteadmin.config.php
            sudo rm *.zip
            sudo mkdir themes
            cd themes
            sudo wget --no-check-certificate http://bitbucket.org/phpliteadmin/public/downloads/phpliteadmin_themes_2013-12-26.zip -a $LOGFILE
            sudo unzip phpliteadmin_themes_2013-12-26.zip 2>&1 | tee -a $LOGFILE
            sudo rm *.zip
...
if [[ $MYMENU == *"nodered"* ]]; then
    ...
    if [ -f /usr/bin/node ] || [ -f /usr/local/bin/node ]; then
        ...
        if [ $(grep public ~/.node-red/settings.js 2> /dev/null | wc -l) -ne 1 ]; then
			...
			node-red-dashboard node-red-node-darksky node-red-contrib-owntracks node-red-contrib-alexa-local 2>&1 | tee -a $LOGFILE
	if [[ $MYMENU == *"hwsupport"* ]]; then
...
if [[ $MYMENU == *"habridge"* ]]; then
    ...
    curl -s https://api.github.com/repos/bwssytems/ha-bridge/releases/latest | jq --raw-output '.assets[0] | .browser_download_url' | wget -a $LOGFILE -i - -O ~/habridge/ha-bridge.jar
    echo -e "[Unit]\n\
...
if [[ $MYMENU == *"grafana"* ]]; then
	...
	test $VERSION_ID = "9" && echo "deb https://repos.influxdata.com/debian jessie stable" | sudo tee -a /etc/apt/sources.list.d/influxdb.list
	...
	[[ $OPSYS == *"UBUNTU"* ]] && [[ $(uname -m) == *"aarch64"* ]] && echo "deb https://dl.bintray.com/fg2it/deb-arm64 stretch main" | sudo tee -a /etc/apt/sources.list.d/grafana.list
	[[ $OPSYS == *"UBUNTU"* ]] && [[ $(uname -m) != *"aarch64"* ]] && echo "deb https://packagecloud.io/grafana/testing/debian/ stretch main" | sudo tee -a /etc/apt/sources.list.d/grafana.list
